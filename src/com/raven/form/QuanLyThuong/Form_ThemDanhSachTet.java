    ...
    public class Form_ThemDanhSachTet extends javax.swing.JPanel {
        ...
        private boolean validateInput() {
            ...
            if (searchText_MaHo.getText().trim().isEmpty()) {
                JOptionPane.showMessageDialog(this, "Mã hộ không được để trống", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return false;
            }
        ...
        private void jButton_XacNhanActionPerformed(java.awt.event.ActionEvent evt) {
            try {
                if (validateInput()) {
                    DanhSachThuongTet newRecord = new DanhSachThuongTet();
                    // Lấy dữ liệu từ form và đặt vào newRecord
                    newRecord.setCCCD(searchText_CCCD.getText().trim());
                    newRecord.setHoTen(searchText_HoVaTen.getText().trim());
                    newRecord.setMaHo(Integer.parseInt(searchText_MaHo.getText().trim()));
                    newRecord.setNgaySinh(new java.sql.Date(jDateChooser_NgaySinh.getDate().getTime()));
                    // Set các trường dữ liệu khác nếu cần
                    Date utilDate = jDateChooser_NgaySinh.getDate();
                    newRecord.setNgaySinh(new java.sql.Date(utilDate.getTime()));
                    DanhSachThuongTetDAO dao = new DanhSachThuongTetDAO();
                    dao.themDanhSachThuongTet(newRecord);
        ...
        private DanhSachThuongTet createNewBonusRecord() {
            DanhSachThuongTet newBonus = new DanhSachThuongTet();

            // Điền thông tin cho đối tượng newBonus từ các trường nhập liệu
            newBonus.setCCCD(searchText_CCCD.getText().trim());
            newBonus.setHoTen(searchText_HoVaTen.getText().trim());
            newBonus.setMaHo(Integer.parseInt(searchText_MaHo.getText().trim()));

            java.util.Date utilDate = jDateChooser_NgaySinh.getDate();
            newBonus.setNgaySinh(new java.sql.Date(utilDate.getTime())); // Convert to java.sql.Date

            // Thêm các thuộc tính khác của newBonus nếu cần
